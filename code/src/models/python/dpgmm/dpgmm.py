...
# modified by Jan Blaha (blahaj22@fel.cvut.cz) 2019
...
class DPGMM:
  ...
  def setPrior(self, mean = None, covar = None, weight = None, scale = 1.0, safe = True):
    ...
    if isinstance(mean, type(None)) or isinstance(covar, type(None)):
      inc = gcp.GaussianInc(self.dims)
      dm = self.getDM()
      for i in xrange(dm.shape[0]): inc.add(dm[i,:])
      ggd = inc.fetch()
      if isinstance(mean, type(None)): mean = ggd.getMean()
      if isinstance(covar, type(None)): covar = ggd.getCovariance() * scale
  ...
  def prob(self, x):
    ...
    # if DM is empty, then we want to take the probability of sample being drawn from priors
    if self.size() == 0:
      return self.priorT.prob(x)
    x = numpy.asarray(x)
    if len(x.shape)==1:
      ret = 0.0
      stick = 1.0
