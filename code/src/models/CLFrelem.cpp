...
void CLFrelem::init(int iMaxPeriod,int elements,int numActivities)
{
	maxPeriod = 7*86400;
	numElements = 7*24;
...
int CLFrelem::add(uint32_t time,float state)
{
	if (measurements == 0)
	{
...
void CLFrelem::update(int modelOrder,unsigned int* times,float* signal,int length)
{
...
void CLFrelem::print(bool verbose)
{
...
float CLFrelem::estimate(uint32_t time)
{
...
float CLFrelem::predict(uint32_t time)
{
	float saturation = 0.01;
	float estimate =  predictGain;
	for (int i = 0;i<order;i++) estimate+=2*predictFrelements[i].amplitude*cos(time/predictFrelements[i].period*2*M_PI-predictFrelements[i].phase);
...
int CLFrelem::save(char* name,bool lossy)
{
...
int CLFrelem::importFromArray(double* array,int len)
{
	int pos = 0;
	type = (ETemporalType)array[pos++];
	if (type != TT_FREMEN && type != TT_LFREMEN) fprintf(stderr,"Error loading the model, type mismatch.\n");
	order = array[pos++];        
	id = array[pos++];
...
int CLFrelem::exportToArray(double* array,int maxLen)
{
	int pos = 0;
	array[pos++] = type;
	array[pos++] = order;
...
int CLFrelem::save(FILE* file, bool lossy)
{
    int pos = 10000;
